...
<link rel="stylesheet" type="text/css"
	href="/pilotpjt/resources/css/admin_notice.css">
...
<body>
	...
	<div class="container">
		...
		<div class="notice-controls">
			...
			{
			%>
			<a href="admin_notice_write" class="write-btn"> <i
				class="fas fa-pen"></i> 공지사항 작성
			</a>
			<%
			}
			%>
			...
			<c:when test="${not empty noticeList}">
				<c:forEach items="${noticeList}" var="notice">
					<div class="notice-card ${notice.fixed ? 'fixed-notice' : ''}">
						<a href="/pilotpjt/notice_detail?id=${notice.id}"
							class="notice-link">
									...
									<div class="meta-left">
										...
										<div class="meta-item">
											<i class="fas fa-calendar meta-icon"></i> <span><fmt:formatDate
													value="${notice.regDate}" pattern="yyyy-MM-dd" /></span>
										</div>
									</div>
									<div class="meta-item">
										<i class="fas fa-eye meta-icon"></i> <span>${notice.viewCount}</span>
									</div>
								</div>
							</div>
						</a>
				...
				<div class="pagination">
					<c:if test="${currentPage > 1}">
						<div class="page-item">
							<a
								href="/pilotpjt/notice_view?page=${currentPage - 1}${not empty param.category ? '&category='.concat(param.category) : ''}${not empty param.sort ? '&sort='.concat(param.sort) : ''}${not empty param.keyword ? '&keyword='.concat(param.keyword) : ''}"
								class="page-link"> <i class="fas fa-chevron-left"></i>
							</a>
						</div>
					</c:if>
					...
					<c:forEach begin="${startPage}" end="${endPage}" var="i">
						<div class="page-item">
							<a
								href="/pilotpjt/notice_view?page=${i}${not empty param.category ? '&category='.concat(param.category) : ''}${not empty param.sort ? '&sort='.concat(param.sort) : ''}${not empty param.keyword ? '&keyword='.concat(param.keyword) : ''}"
								class="page-link ${currentPage == i ? 'active' : ''}"> ${i}
							</a>
						</div>
					</c:forEach>
